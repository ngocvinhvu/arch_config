...
def user_add():
	...
	user_orgs_cols.insert({"user_id":user_id, "role_name":"admin"})
...
def router_add(user_id):
	...
	configurations_cols = mongo.db.configurations
	...
	if brandname == "meraki" or "Meraki":
		_id  = routers_cols.insert_one({"router_brand_id": 1, "status":"active"})
		router_id = str(_id.inserted_id)
		routers_cols.update_one({"_id":_id.inserted_id}, {"$set": {"id":router_id}})
		router_organizations_cols.insert_one({"router_id":router_id, "organizations_id":1})
	return	{
			'status': 1,
			'brand_short_name': brandname,
			}
@app.route('/v1/routers/<router_id>', methods=['DELETE'])
def router_del(user_id,router_id):
	routers_cols = mongo.db.routers
...
@app.route('/v1/user_id/<int:user_id>/routers/list', methods = ['GET'])
def router_list(user_id):
	cols = mongo.db.routers
	router_data = cols.find({"user_id":user_id}).toArray()
	router_ids = [router_data[i]['router_id'] for i in range(len(router_data))]
	router_brandnames = [router_data[i]['brandname'] for i in range(len(router_data))]
	return {
			'status': 1,
			'router_list' : dict(zip(router_ids, router_brandnames))
			}
...
def get_user(user_id):
	...
	if not user:
		return {
				"status": 0,
				"msg": "user_id is not existing"
				}
	return {"status": 1,
			"msg": f"user_id: {user.user_id} is existing"
			}



@app.route('/api/users/<user_id>/routers/<router_id>/api_key/add', methods = ['POST']) # set api
def api_key_add(user_id):
